...
class StatsService:
    ...
    async def _get_discipline_filters(
        self, course: Optional[int], semester: Optional[int]
    ) -> list[int]:
        where = []
    ...
    async def get_average(
        ...
        if discipline_id:
            ...
            if discipline_ids:
                mark_where.append(
                    self.mark_repo.model.discipline_id.in_(discipline_ids)
                )

        if period_start and period_end:
            mark_where.append(
                self.mark_repo.model.date.between(period_start, period_end)
            )
